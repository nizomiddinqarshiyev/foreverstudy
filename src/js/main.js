...
document.addEventListener('DOMContentLoaded', function() {
    // Testimonial carousel: 1 slide per move, always center
    ...
    function updateSlider() {
        ...
        // Always show only 1 slide per move, center it
        const videoWidth = videos[0].offsetWidth + parseInt(getComputedStyle(track).gap || 24);
        // Limit index
        if (currentIndex < 0) currentIndex = videos.length - 1;
        if (currentIndex >= videos.length) currentIndex = 0;
        // Center the active slide
        const containerWidth = track.parentElement.offsetWidth;
        const offset = (containerWidth - videoWidth) / 2 - currentIndex * videoWidth;
